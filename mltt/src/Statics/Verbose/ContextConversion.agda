...
conv-≡ty Γ≡Γ′ (Ǔ-T Γ-wf) = Ǔ-T (presup-≡ctx Γ≡Γ′ .snd)
conv-≡ty Γ≡Γ′ (Ť-T M-wf) = Ť-T (conv-tm Γ≡Γ′ M-wf)
...
conv-tm Γ≡Γ′ (Ǔ-wf Γ-wf) = Ǔ-wf (presup-≡ctx Γ≡Γ′ .snd)
conv-tm Γ≡Γ′ (Ť-wf M-wf) = Ť-wf (conv-tm Γ≡Γ′ M-wf)
...
conv-≡tm Γ≡Γ′ (Ǔ-cong Γ-wf) = Ǔ-cong (presup-≡ctx Γ≡Γ′ .snd)
conv-≡tm Γ≡Γ′ (Ť-cong M-eq) = Ť-cong (conv-≡tm Γ≡Γ′ M-eq)
...
conv-≡tm Γ≡Γ′ (ℕ̇-β-s· P₀ H₁ H₂ H₃) = ℕ̇-β-s· (conv-ty Γ,ℕ̇≡Γ′,ℕ̇ P₀) (conv-tm Γ≡Γ′ H₁) (conv-tm Γ,ℕ̇,C≡Γ′,ℕ̇,C H₂) (conv-tm Γ≡Γ′ H₃)
  where
    ...
    Γ,ℕ̇≡Γ′,ℕ̇ = ,ctx-cong′ Γ≡Γ′ (ℕ̇-wf Γ-ctx) (ℕ̇-wf Γ′-ctx)
    Γ,ℕ̇,C≡Γ′,ℕ̇,C = ,ctx-cong′ Γ,ℕ̇≡Γ′,ℕ̇ P₀ (conv-ty Γ,ℕ̇≡Γ′,ℕ̇ P₀)
conv-≡tm {Γ = Γ} {Γ′ = Γ′} Γ≡Γ′ (Π̌-Ť {M = M} {i = i} M-wf N-wf) = Π̌-Ť M-wf′ (conv-tm Γ,T[M]≡Γ′,T[M] N-wf)
  where
    M-wf′ = conv-tm Γ≡Γ′ M-wf
    Γ,T[M]≡Γ′,T[M] : Γ , T (toℕ i) M ≡ Γ′ , T (toℕ i) M ctx
    Γ,T[M]≡Γ′,T[M] = ,ctx-cong′ Γ≡Γ′ (T-wf M-wf) (T-wf M-wf′)
conv-≡tm Γ≡Γ′ (ℕ̌-Ť Γ-wf) = ℕ̌-Ť (presup-≡ctx Γ≡Γ′ .snd)
conv-≡tm Γ≡Γ′ (Ǔ-Ť Γ-wf) = Ǔ-Ť (presup-≡ctx Γ≡Γ′ .snd)
conv-≡tm Γ≡Γ′ (Ť-Ť M-wf) = Ť-Ť (conv-tm Γ≡Γ′ M-wf)
...
conv-≡tm Γ≡Γ′ (Ǔ-[] σ-wf) = Ǔ-[] (conv-subst Γ≡Γ′ σ-wf)
conv-≡tm Γ≡Γ′ (Ť-[] M-wf σ-wf) = Ť-[] M-wf (conv-subst Γ≡Γ′ σ-wf)
