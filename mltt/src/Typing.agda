...
data _⊢_≡_⦂_tm where
  ...
  Π̇-η        : (H₀ : Γ ⊢ M ⦂ Π̇ A B tm) →
                     Γ ⊢ M ≡ ƛ ((M [ ↑ ]) · (# zero)) ⦂ Π̇ A B tm
  -- ℕ̇
  ℕ̇-β-z·     : (H₀ : Γ , ℕ̇ ⊢ C ty) →
               (H₁ : Γ ⊢ L ⦂ C [ I , z· ] tm) →
               (H₂ : Γ , ℕ̇ , C ⊢ M ⦂ C [ (↑ ∗ ↑) , (s· # zero) ] tm) →
